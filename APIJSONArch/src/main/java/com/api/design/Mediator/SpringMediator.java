 ...
 */
@Component
public class SpringMediator implements Mediator,IPublish {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public <T> UUID usend(Request<T> request) {
        if (request == null) {
            throw new NullPointerException("The given request object cannot be null");
        }
        ...
        return  requestHandler.uhandle(request);
    }
    
    
    

    public <TNotification extends Notification> Notification publishNotification(TNotification notification) {
        // implementation of publishNotification method goes here
